...
{
    /**
     * Listar cuidadores profissionais
     */
    public function index(Request $request)
    {
        try {
            $query = User::where('profile', 'professional_caregiver')
            ...
            $caregivers = $query->get()->map(function ($caregiver) {
                ...
                return [
                    ...
                    'courses' => $caregiver->caregiverCourses->map(function ($course) {
                        return [
                            'id' => $course->id,
                            'name' => $course->name,
    ...
    /**
     * Mostrar detalhes de um cuidador profissional
     */
    public function show($id)
    {
        try {
            $caregiver = User::where('id', $id)
                ->where('profile', 'professional_caregiver')
                ->with(['caregiverCourses', 'caregiverReviews.author'])
                ->first();
            ...
            $reviews = $caregiver->caregiverReviews->map(function ($review) {
                return [
                    'id' => $review->id,
    ...
    /**
     * Criar avaliação de um cuidador profissional
     */
    ...
    /**
     * Listar clientes do cuidador profissional
     */
    public function getClients(Request $request)
    {
        try {
            ...
            // Verificar se é cuidador profissional (usando campo profile)
            if ($user->profile !== 'professional_caregiver') {
                // Tentar verificar novamente no banco
                $user = User::find($user->id);
                if (!$user || $user->profile !== 'professional_caregiver') {
                    return response()->json([
                        'success' => false,
                        'message' => 'Acesso negado. Você não é um cuidador profissional.',
                    ], 403);
                }
            }
            ...
            // Buscar IDs de usuários que são cuidadores profissionais para excluir
            $professionalCaregiverIds = User::where('profile', 'professional_caregiver')
                ->pluck('id')
                ->toArray();

            // Buscar clientes que estão nos mesmos grupos (excluindo o próprio cuidador e outros cuidadores profissionais)
            ...
            $clients = User::whereIn('id', $clientIds)
                ...
                ->map(function ($client) use ($user) {
                    ...
                    // O accessor retorna asset('storage/' . $this->photo) se existir, ou null
                    ...
                    return [
                        ...
                        'name' => $client->name,
                        'photo' => $photoUrl,
                        'photo_url' => $photoUrl, // Adicionar photo_url para compatibilidade com frontend
                        'city' => $client->city,
