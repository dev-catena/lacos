...
Criei um script **ultra agressivo** que intercepta **TODAS** as saídas do Expo e substitui qualquer referência a `localhost` pelo IP correto (`192.168.100.10`).
...
3. **Substitui TODOS os padrões de localhost**:
   - `http://localhost:8081` → `exp://192.168.100.10:8081`
   - `https://localhost:8081` → `exp://192.168.100.10:8081`
   - `exp://localhost:8081` → `exp://192.168.100.10:8081`
   - `localhost:8081` → `192.168.100.10:8081`
   - `127.0.0.1:8081` → `192.168.100.10:8081`
...
3. Cole: `exp://192.168.100.10:8081`
4. Conecte
...
Mesmo que o Expo gere URLs com localhost internamente, **todas serão substituídas automaticamente** antes de aparecerem no terminal. O Metro bundler estará acessível no IP correto (`192.168.100.10:8081`).
...
curl http://192.168.100.10:8081/status
