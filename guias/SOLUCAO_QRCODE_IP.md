...
O QR code estava gerando `localhost:8081` (que não funciona no celular), mas o IP correto é `192.168.100.10`.
...
- ✅ Forçar IP `192.168.100.10` com `--host`
...
```json
"start": "expo start --lan --host 192.168.100.10 --port 8081",
"start:lan": "expo start --lan --host 192.168.100.10 --port 8081"
```
...
```bash
cd /home/darley/lacos
npx expo start --lan --host 192.168.100.10 --port 8081 --clear
```
...
2. Verifique no terminal que aparece `exp://192.168.100.10:8081`
...
Deve mostrar `192.168.100.10` (ou outro IP da sua rede local).

### Se o IP Mudar

Se o IP da sua máquina mudar, atualize:

1. **No script** `INICIAR_EXPO_FIX.sh`: linha com `EXPO_IP="192.168.100.10"`
...
- ✅ O IP `192.168.100.10` é o IP local da sua máquina na rede
- ✅ Não use `localhost` ou `127.0.0.1` no celular (não funciona)
- ✅ Use sempre o IP da rede local (`192.168.100.10`)
