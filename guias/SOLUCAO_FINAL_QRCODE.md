...
O QR code continua mostrando `localhost:8081` mesmo após usar `--host 192.168.100.10`.
...
- ✅ Exporta `REACT_NATIVE_PACKAGER_HOSTNAME=192.168.100.10`
- ✅ **Passa a variável diretamente no comando** `expo start`
- ✅ Usa `--lan --host 192.168.100.10 --port 8081`
...
REACT_NATIVE_PACKAGER_HOSTNAME=192.168.100.10 npx expo start --lan --host 192.168.100.10 --port 8081 --clear
```
...
REACT_NATIVE_PACKAGER_HOSTNAME=192.168.100.10 npx expo start --lan --host 192.168.100.10 --port 8081 --clear --dev-client
```
...
npx expo start --lan --host 192.168.100.10
```

**✅ FUNCIONA:**
```bash
REACT_NATIVE_PACKAGER_HOSTNAME=192.168.100.10 npx expo start --lan --host 192.168.100.10
```
...
E o QR code deve conter: `exp://192.168.100.10:8081`
...
2. Verifique que aparece `exp://192.168.100.10:8081` no terminal
...
- ✅ O IP `192.168.100.10` é o IP local da sua máquina
