...
const getBaseURL = () => {
  if (Platform.OS === 'android') {
    return 'http://192.168.100.10:8000/api'; // IP da máquina
...
Criar arquivo `.env`:
```
API_BASE_URL=http://192.168.100.10:8000/api
```
...
```bash
curl -X POST http://192.168.100.10:8000/api/login \
