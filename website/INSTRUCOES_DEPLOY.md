...
scp -P 63022 -r dist/* darley@192.168.100.10:/var/www/lacos-website/
...
Verifique os logs:
```bash
ssh -p 63022 darley@192.168.100.10 'sudo nginx -t'
```

### Site não carrega
1. Verifique se o Nginx está rodando:
```bash
ssh -p 63022 darley@192.168.100.10 'sudo systemctl status nginx'
```

2. Verifique os logs:
```bash
ssh -p 63022 darley@192.168.100.10 'sudo tail -f /var/log/nginx/lacosapp-error.log'
```

3. Verifique se os arquivos estão no lugar certo:
```bash
ssh -p 63022 darley@192.168.100.10 'sudo ls -la /var/www/lacos-website'
```
...
1. **API Backend**: O site está configurado para se conectar automaticamente ao backend:
   ...
   - Em desenvolvimento: `http://192.168.100.10/api`
