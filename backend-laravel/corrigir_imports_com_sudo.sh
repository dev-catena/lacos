...
echo "🔧 Corrigindo imports no CaregiverController (com sudo)..."
echo ""
...
if ! grep -q "use Illuminate\\Support\\Facades\\Auth;" "$CONTROLLER_FILE"; then
    ...
    # Adicionar após use Illuminate\Support\Facades\DB;
    sudo sed -i "/^use Illuminate\\Support\\Facades\\DB;/a use Illuminate\\Support\\Facades\\Auth;" "$CONTROLLER_FILE"
...
if ! grep -q "use Illuminate\\Support\\Facades\\Log;" "$CONTROLLER_FILE"; then
    ...
    sudo sed -i "/^use Illuminate\\Support\\Facades\\Auth;/a use Illuminate\\Support\\Facades\\Log;" "$CONTROLLER_FILE"
...
echo ""
echo "🧪 Agora teste o endpoint no app!"
