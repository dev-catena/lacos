...
class AdminDoctorController extends Controller
{
    ...
    {
        try {
            $query = User::where('profile', 'doctor')
                ->where(function($q) {
                    $q->whereNull('doctor_approved_at')
                      ->orWhere('doctor_approved_at', '=', '0000-00-00 00:00:00');
                });
            
            // Verificar is_blocked apenas se a coluna existir
        ...
        try {
            $doctors = User::where('profile', 'doctor')
                ...
                ->map(function($doctor) {
                    ...
                    $isBlocked = false;
                    if (Schema::hasColumn('users', 'is_blocked')) {
                        $isBlocked = (bool) ($doctor->is_blocked ?? false);
                    }
    ...
    public function approve($id)
    {
        try {
            ...
            // Construir array de update condicionalmente
            $updateData = [
                'doctor_approved_at' => now(),
                'updated_at' => now()
            ];
            
            // Adicionar campos de ativação apenas se as colunas existirem
            if (Schema::hasColumn('users', 'doctor_activation_token')) {
                $updateData['doctor_activation_token'] = $activationToken;
            }
            if (Schema::hasColumn('users', 'doctor_activation_token_expires_at')) {
                $updateData['doctor_activation_token_expires_at'] = $expiresAt;
            }
            
            // Adicionar is_blocked apenas se a coluna existir
            if (Schema::hasColumn('users', 'is_blocked')) {
    ...
    private function sendActivationEmail($doctor, $token)
    {
        try {
            ...
            $baseUrl = config('backend.base_url') ?: config('app.url');
            ...
            try {
                \Log::info('Tentando enviar via Mail::send() do Laravel para: ' . $doctor->email);
                ...
                \Log::info('✅ Email de ativação enviado via Laravel Mail para: ' . $doctor->email);
                
                // Se o driver for 'log', avisar que email não foi enviado de verdade
                if ($mailDriver === 'log') {
                    \Log::warning('⚠️ Driver "log" detectado - email não foi enviado, apenas logado. Configure SMTP no .env para enviar emails reais.');
                }
                
                return;
            } catch (\Exception $mailException) {
                \Log::error('Erro no Mail::send() do Laravel: ' . $mailException->getMessage(), [
    ...
    public function reject($id)
    {
        try {
            ...
            // Atualizar is_blocked apenas se a coluna existir
            if (Schema::hasColumn('users', 'is_blocked')) {
    ...
    public function block($id)
    {
        try {
            ...
            \Log::info('Médico bloqueado', [
                'doctor_id' => $doctor->id,
                'email' => $doctor->email,
                'is_blocked' => Schema::hasColumn('users', 'is_blocked') ? $doctor->is_blocked : 'N/A',
            ]);
            ...
            return response()->json([
                ...
                'doctor' => [
                    ...
                    'name' => $doctor->name,
                    'email' => $doctor->email,
                    'is_blocked' => Schema::hasColumn('users', 'is_blocked') ? (bool) $doctor->is_blocked : false,
                ]
            ]);
        } catch (\Exception $e) {
            \Log::error('Erro ao bloquear médico', [
                'doctor_id' => $id,
                'error' => $e->getMessage(),
    ...
    public function activate(Request $request)
    {
        ...
        try {
            ...
            // Limpar campos de ativação apenas se as colunas existirem
            if (Schema::hasColumn('users', 'doctor_activation_token')) {
                $updateData['doctor_activation_token'] = null;
            }
            if (Schema::hasColumn('users', 'doctor_activation_token_expires_at')) {
                $updateData['doctor_activation_token_expires_at'] = null;
            }
            ...
            // Verificar se realmente foi atualizado (apenas se as colunas existirem)
            $doctorCheck = null;
            if (Schema::hasColumn('users', 'doctor_activation_token') && Schema::hasColumn('users', 'doctor_activation_token_expires_at')) {
