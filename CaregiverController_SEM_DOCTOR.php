...
{
    /**
     * Listar cuidadores profissionais (SEM médicos)
     */
    public function index(Request $request)
    {
        try {
            // Filtrar APENAS cuidadores profissionais (não médicos)
    ...
    /**
     * Mostrar detalhes de um cuidador profissional (SEM médicos)
     */
    public function show(Request $request, $id)
    {
        try {
            $caregiver = User::where('profile', 'professional_caregiver')
                ->with(['caregiverCourses', 'caregiverReviews'])
                ->find($id);
    ...
    public function createReview(Request $request, $id)
    {
        try {
            ...
            // Buscar cuidador
            $caregiver = User::where('profile', 'professional_caregiver')->find($id);
            if (!$caregiver) {
                return response()->json([
                    'success' => false,
                    'message' => 'Cuidador profissional não encontrado',
    ...
    /**
     * Listar clientes de um cuidador profissional
     */
    public function getClients(Request $request)
    {
        try {
            ...
            // Verificar se é cuidador profissional (SEM médicos)
            ...
            $clients = User::whereIn('id', $clientIds)
                ...
                ->map(function ($client) use ($user) {
                    ...
                    // Usar o accessor photo_url do modelo User
    ...
    public function getClientDetails(Request $request, $id)
    {
        try {
            ...
            if (empty($commonGroups)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Você não tem permissão para ver este cliente',
                ], 403);
            }

            // Buscar avaliações do cuidador para este cliente
            ...
            return response()->json([
                'success' => false,
                'message' => 'Erro ao buscar detalhes do cliente: ' . $e->getMessage(),
            ], 500);
        }
    }
