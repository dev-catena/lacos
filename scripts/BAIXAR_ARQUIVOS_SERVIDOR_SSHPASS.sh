...
sshpass -p "$PASSWORD" ssh -p $PORT -o StrictHostKeyChecking=no darley@192.168.100.10 << 'ENDSSH'
...
sshpass -p "$PASSWORD" scp -P $PORT -o StrictHostKeyChecking=no darley@192.168.100.10:/tmp/lacos_backup.tar.gz /tmp/lacos_backup.tar.gz
...
sshpass -p "$PASSWORD" ssh -p $PORT -o StrictHostKeyChecking=no darley@192.168.100.10 "rm -f /tmp/lacos_backup.tar.gz" 2>/dev/null
