...
#
# (Há cópia idêntica em backend-laravel/scripts/deploy-web-admin-nginx-ssl.sh.)
#
# Uso no servidor (Ubuntu/Debian, como root ou com sudo):
#   export CERTBOT_EMAIL="seu-email@lacosapp.com"
#   # Produção: URL da API já vem de web-admin/.env.production (VITE_PRODUCTION_API_BASE_URL).
#   # Rede local no build: exporte antes do npm run build se precisar:
#   export VITE_PRODUCTION_API_BASE_URL="https://gateway.lacosapp.com/api"
...
echo "Se a API mudar de URL, edite web-admin/.env.production (VITE_PRODUCTION_API_BASE_URL) e rode o build outra vez."
