...
# Script para substituir referências hardcoded ao IP 192.168.100.10 por variáveis de ambiente
...
if grep -q "192.168.100.10" src/config/api.js 2>/dev/null; then
...
HARDCODED_WEBSITE=$(grep -c "192.168.100.10" website/src/config/api.js 2>/dev/null || echo 0)
...
HARDCODED_ADMIN=$(grep -c "192.168.100.10" web-admin/src/config/api.js 2>/dev/null || echo 0)
...
HARDCODED_BACKEND=$(grep -c "192.168.100.10" backend-laravel/app/Http/Controllers/Api/AdminDoctorController.php 2>/dev/null || echo 0)
